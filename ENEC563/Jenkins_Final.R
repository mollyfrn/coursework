...
# Questions:
# 1.1) Refit your final egg count rate model from Assignment 1 as a Bayesian model. This should be a model with a normally distributed response in which there is a two-factor interaction between Density and Season.
# 1.1a) Provide diagnostics that indicate that the Markov chains have converged to the corresponding posterior distributions and are properly mixing.
# (Hint, recall that the library rstanarm has equivalent functions for lm,glm,lmer,glmer where you just need to add stan_ to the beginning of those functions.  e.g. stan_lm).
...
# 1.4) Obtain estimates of the six treatment means as well as 95% percentile credible intervals for these means. Compare these values to what you obtained in Assignment 2. (You may use the key for Assignment 2 if you wish.)
# Hint: For question 1 you may get an error for fitting your model that says "'location' must be specified".  For some reason, the stan_lm function requires a prior argument.  Use prior=NULL, to remove this error.
# 
# 
# 
# Problem 2: 
...
# Questions:
# 2.1) Identify as many of the important statistical experimental design features of this experiment that you can. Be sure to use the language we've presented in this course when naming these design features.
...
# 2.2) Fit a sensible starting model for this experiment that properly accounts for all the design features you mentioned in Question 1.
...
# 2.4) Obtain estimates of the mean growth rates during each time period separately by temperature and obtain 95% confidence intervals for the means. Carry out these calculations for a typical coral animal with an initial surface area of 10 cm2, a value that is close to the mean initial size of animals used in the study.
...
# Hints 1&2 add up to including a model to replace surf.area with I(surf.area-10) in your original model and removing the intercept from your model . You can then use intervals(newmodelname,which="fixed") to calculate the mean and confidence intervals for all treatment combinations.   Hint 3: You can also set up a contrast matrix and get the standard errors with a sandwich expression involving the variance-covariance matrix.
# 2.5) Prepare a suitable graph that shows the individual treatment means and the confidence intervals that you calculated in Question 4 and permits easy comparison of the effect of temperature on the mean growth rates over time.
...
# Questions:
# 3.1) A single regression model involving the predictors Distance and Morph can answer the researcher's question of interest. Write down that regression model in generic form. What I want here is the expression that would appear on right side of a regression equation written out as a sum of parameters times predictors.
# Hint 1: I'm looking for an expression of the form ??0 + ??1x + ., where you should include as many terms as are needed to describe the basic outlines of the experiment and answer the researcher's question. Be sure to identify what the variables in your expression represent.
# Hint 2: I'm not asking you to include the complications discussed in Question 6 at this point.
# 3.2) Using the expression you've written as your answer to Question 1, state a null hypothesis in terms of model parameters that directly tests whether the relationship between predation rate and distance is the same for the two morphs.
# 3.3) Given the nature of the response variable, fit an appropriate regression model that addresses the researcher's primary question.
# 3.4) Test the overall fit of the model of Question 3 using an appropriate goodness of fit test. Verify that the test is appropriate.
# 3.5) There's a structural characteristic of these data that we've been ignoring that may be acting to make the data heterogeneous. The structure is represented by a variable in the data set. What am I talking about?
# 3.6) Refit your model from Question 3 but this time also account for the structure of the data. In reference to this structure, which variable in your model is a level-1 variable and which variable is a level-2 variable?
# 3.7) The statistical evidence for this structure turns out to be very weak. Demonstrate this either by carrying out a formal statistical test or by citing relevant statistics. (Note: We'll continue to use the model with structure in the remaining questions anyway.)
#Hint 1: If you elect to carry out a statistical test using the likelihood you need to be aware that in the hypothesis test you're carrying out, H0: ??2 = 0, zero is a boundary value for ??2. The usual distribution of the likelihood ratio statistic is incorrect for boundary values. The p-value adjustment that we used for testing H0: ?? = 0 in a negative binomial model (Page 3 on  lecture 17) is the same adjustment you need to carry out here.
# 3.8) Using the model from question 6, compute a statistic that compares the odds of being removed 50 km away from Liverpool to the odds of being removed in Liverpool (0 km away). Calculate this statistic separately for the dark and light morphs and interpret your results.
# 3.9) Using the model from question 6, produce a graph that summarizes the results of the analysis as follows.
#   a) Plot the empirical proportions (the observed proportions of moths eaten) as a function of distance. Distinguish the plotted values by their Morph type.
#   b) Plot the predicted probability of being eaten as a function of distance using only the fixed effect estimates from your model. Display these as curves superimposed on your scatter plot of empirical probabilities
#   c) Plot the predicted probability of being eaten as a function of distance using both the fixed effect estimates and the random effect predictions. Plot these as points being sure to distinguish them from the points you plotted in (a).
#   d) Label your diagram appropriately using a coherent set of colors and symbol types.
# 3.10) It was mentioned in the background section above that the data we are using are tabulated versions of the raw data from each location for each morph. The actual observations are the number of moths removed out of a fixed total on separate trees at a given location. The results for individual trees were then combined to yield the data we used for this analysis. Which of the four basic assumptions of the probability model that we've been using is the one that is most likely to be (but not necessarily) violated by combining data in this fashion?                                                                                                                                                                             